...
import kr.or.kosa.utils.Edu_String;
...
public class Ex09_Calendar {
	public static void main(String[] args) {
		...
		System.out.println(dt.toString());  //Wed Sep 14 16:56:12 KST 2022
		...
		//java.util.GregorianCalendar[time=1663142237936,areFieldsSet=true,areAllFieldsSet=true,lenient=true,zone=sun.util.calendar.ZoneInfo[id="Asia/Seoul",offset=32400000,dstSavings=0,useDaylight=false,transitions=30,lastRule=null],firstDayOfWeek=1,minimalDaysInFirstWeek=1,ERA=1,YEAR=2022,MONTH=8,WEEK_OF_YEAR=38,WEEK_OF_MONTH=3,DAY_OF_MONTH=14,DAY_OF_YEAR=257,DAY_OF_WEEK=4,DAY_OF_WEEK_IN_MONTH=2,AM_PM=1,HOUR=4,HOUR_OF_DAY=16,MINUTE=57,SECOND=17,MILLISECOND=936,ZONE_OFFSET=32400000,DST_OFFSET=0]
		
		
		System.out.println("년도 : "+ cal.get(Calendar.YEAR)); 
		System.out.println("월 : "+ cal.get((Calendar.MONTH))+1);  //0월 ~ 11월  +1해야 우리나라와 맞음
		System.out.println("일 : "+ cal.get(Calendar.DATE));
		
		System.out.println("이 달의 몇째주 : "+ cal.get(Calendar.WEEK_OF_MONTH));
		System.out.println("이달의 몇일이니  : "+ cal.get(Calendar.DAY_OF_MONTH));
		System.out.println("이 주의 몇일이니 : "+ cal.get(Calendar.DAY_OF_WEEK)); //일요일부터 계산해서 4일
		...
		//학사 관리 시스템
		//웹 : 페이지 본수 : 200page
		//150개 page 날짜 정보 각 페이지 하단에 출력 (2022년09월14일)
		//A 친구 >> cal.get(Calendar.YEAR) + "월" ...  
		//갑자기 해외수출하게 됨 .... 날짜 한글말고 2022-09-14로 쓰자 -> 150페이지 다 일일히 해야함
		
		//한번만 수정해서 150페이지의 날짜 정보가 수정 될려면 클래스,함수만들어서 불러서 쓰기
		
		System.out.println(Edu_String.DateString(Calendar.getInstance()));
		...
		//Quiz
		System.out.println(Edu_Date.DateString(Calendar.getInstance()));  //2022년09월14일
	}
